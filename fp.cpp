...
T** create_matr(size_t rows, size_t cols) {
	T** matr = new T * [rows];//ñîçäà¸ì ìàññèâ óêàçàòåëåé íà îäíîìåðíûå ìàññèâû
	for (size_t i = 0; i < rows; i++)
	{
		matr[i] = new T[cols];
	}
...
void delete_matr(T** matr, size_t rows) {
	//ñíà÷àëà óäàëÿåì äàííûå âíóòðè ñòðîê
	for (size_t i = 0; i < rows; i++)
	{
		delete[] matr[i];
	}
...
int Action(int** matr, size_t rows, size_t cols, int (*act) (int**, size_t, size_t)) {
	return act(matr, rows, cols);//âûçûâûåì îäíó èç ôóíêöèé
}
...
int main() {
	...
	int (*act[3]) (int**, size_t, size_t) = {min, max, avg}; //óêàçàòåëü íà îäíó èç ôóíêöèé (min, max èëè avg)
